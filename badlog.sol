...
contract InsultLogger {
    event LogMessage(string message, string insult);
    event ErrorMessage(string message, string insult);

    string[] private logInsults = [
    ...
    function getRandomItem(string[] memory array) private view returns (string memory) {
        uint256 index = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % array.length;
        return array[index];
    }

    function logWithInsult(string memory message) public {
        emit LogMessage(message, getRandomItem(logInsults));
    }

    function errorWithInsult(string memory message) public {
        emit ErrorMessage(message, getRandomItem(errorInsults));
    }
}
